...
test("check account controller", () => {
    ...
    expect(myAccount.accountArray[2].name).toEqual("RSP");
    ...
    expect(myAccount.mostValuableAccount()).toEqual( {"currentBlance": 2000, "name": "Study plan"});
    expect(myAccount.leastValuableaccount()).toEqual({"currentBlance": 200, "name": "ESP"});
});
    test('getAccount(name) returns account by name', () => {
    const myAccount = new AccountController("RSP",200);
    expect(myAccount.getAccount("RSP")).toEqual({"currentBlance": 200, "name": "RSP"});
...
test('removeAccount() removes account of that name', () => {
    ...
    myAccount.removeAccount("ESP");
    expect(myAccount.getAccounts().length).toEqual(accountCount - 1);
    expect(myAccount.getAccounts()).toEqual([{"currentBlance": 500, "name": "Savings",}]);
